...
db.execute(
  "CREATE TABLE IF NOT EXISTS general(id  INTEGER PRIMARY KEY autoincrement not null,title TEXT,desc TEXT, email TEXT, pass TEXT,account TEXT,pin TEXT, url TEXT)",
);
db.close();
...
exports.add = function (
  ...
  var db = Ti.Database.open("people");
  db.execute(
    "INSERT INTO general (title,desc,email,pass,account,pin,url) VALUES(?,?,?,?,?,?,?)",
    title,
    desc,
    email,
    pass,
...
exports.updateinfo = function (
  ...
  var db = Ti.Database.open("people");
  db.execute(
    "UPDATE general set title=?,desc=?,email=?,pass=?,account=?,pin=?,url=? where id=?",
    title,
    desc,
    email,
    pass,
