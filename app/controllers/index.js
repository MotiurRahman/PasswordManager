  ...
  if (pass.length > 0) {
    if ($.pin.value == pass[0].pass) {
      if (OS_ANDROID) {
        ...
        Alloy.createController("info").getView().open({transition: Ti.UI.iOS.AnimationStyle.FLIP_FROM_LEFT});
      }
    } else {
      if (OS_ANDROID) {
        var toast = Ti.UI.createNotification({message: "Pleaes provide a correct password!", duration: Ti.UI.NOTIFICATION_DURATION_LONG});
        toast.show();
      } else {
    ...
    if (OS_ANDROID) {
      var toast = Ti.UI.createNotification({message: "Please create a password", duration: Ti.UI.NOTIFICATION_DURATION_LONG});
      toast.show();
    } else {
...
function onCreate(e) {
  ...
  if (pass > 0) {
    if (OS_ANDROID) {
      var toast = Ti.UI.createNotification({message: "You already have a password!", duration: Ti.UI.NOTIFICATION_DURATION_LONG});
      toast.show();
    } else {
